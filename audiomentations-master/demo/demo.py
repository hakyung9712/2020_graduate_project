...
if __name__ == "__main__":
    ...
    zero=[]
    LOAD_DIR=os.path.dirname("C:/baby_crying/audiomentations-master/audiomentations-master/demo/input/")
    files =os.listdir(LOAD_DIR)
    #print(files)

    for filename in glob.glob(os.path.join(LOAD_DIR,'*.wav')):
        samplerate,sound_np=wavfile.read(filename)
        if sound_np.dtype != np.float32:
            assert sound_np.dtype == np.int16
            sound_np = sound_np / 32767  # ends up roughly between -1 and 1
        zero.append(sound_np)
    ...
    for j in range(len(zero)):
        ...
        for i in range(6):
            output_file_path = os.path.join(
                #output_dir, "AddBackgroundNoise_{:03d}.mp3".format(i)
                output_dir, "AddBackgroundNoise_{1}_{0}.wav".format(i,j)
            )
