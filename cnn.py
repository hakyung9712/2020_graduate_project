...
def train(args):
    ...
    #Sequential은 레이어를 선형으로 연결하여 구성함
    #Conv1D와 MaxPooling1D층을 쌓고 전역풀링층으로 마무리
    model = Sequential()
    #합성곱을 활용한 순서형 데이터 분류
    #출력뉴런 갯수가 첫째인수, activation인수로 활성화함수 설정
    #64개의 마스크를 사용하겠다는 뜻.행열이 193,1
    model.add(Conv1D(64, 3, activation='relu', input_shape=(193, 1)))
    model.add(Conv1D(64, 3, activation='relu'))
    #결과가 여전히 크고 복잡할때 축소하는 것. 가장 큰 값만 다음층으로 넘기고 나머지 버림
    model.add(MaxPooling1D(3))
    model.add(Conv1D(128, 3, activation='relu'))
    model.add(Conv1D(128, 3, activation='relu'))
    #시간축 전체에 풀링을 적용(서브샘플링)
    model.add(GlobalAveragePooling1D())
    #랜덤한 노드를 꺼주어 학습데이터에 지나치게 치우쳐서 학습되는 과적합 방지.50%끄는 것
    model.add(Dropout(0.5))
    model.add(Dense(class_count, activation='softmax'))
    #loss인수로 비용함수 설정. optimizer 인수로 최적화알고리즘 설정. metrics 인수로 
    #트레이닝 단계에서 기록할 성능 기준 설정
    ...
    if op.exists(args.model):
        ...
        while True:
            try:
                ...
                for p in pred:
                    if(p==0): print("Dog bark")
                    elif(p==1):print("Rain")
                    elif(p==2):print("Sea waves")
                    elif(p==3):print("Baby cry")
                    elif(p==4):print("Clock")
                    elif(p==5):print("Person sneeze")
                    elif(p==6):print("Helicopter")
                    elif(p==7):print("Chainsaw")
                    elif(p==8):print("Rooster")
                    elif(p==9):print("Fire crackling")
